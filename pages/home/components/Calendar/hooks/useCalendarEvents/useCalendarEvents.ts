...
const useCalendarEvents = ({
  ...
  const currentMonthEvents = useMemo(() => {
    return events.filter((event) => {
      ...
      return (
        // El evento comienza en el mes y año actuales
        (eventStartYear === currentYear && eventStartMonth === currentMonth) ||
        // El evento termina en el mes y año actuales
        (eventEndYear === currentYear && eventEndMonth === currentMonth) ||
        // El evento abarca el mes actual (comienza antes y termina después)
  ...
  const selectedDateEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      // Comparamos las fechas ignorando las horas
  ...
  // Manejador para cuando se hace clic en una fecha
